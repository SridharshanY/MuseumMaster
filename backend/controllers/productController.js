...
const getProducts = async (req, res, next) => {
  try {
    ...
    if (!products || products.length === 0) {
      res.statusCode = 404;
      throw new Error('No Exhibits are presented currently. Try some time later');
    }
...
const createProduct = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ message: 'Exhibit added', createdProduct });
  } catch (error) {
    next(error);
  }
};
...
const updateProduct = async (req, res, next) => {
  try {
    ...
    if (!product) {
      res.statusCode = 404;
      throw new Error('No such exhibit found!');
    }
    ...
    res.status(200).json({ message: 'Exhibit data updated', updatedProduct });
  } catch (error) {
    next(error);
  }
};
...
const deleteProduct = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ message: 'Exhibit deleted' });
  } catch (error) {
    next(error);
  }
};
...
const createProductReview = async (req, res, next) => {
  try {
    ...
    if (alreadyReviewed) {
      res.statusCode = 400;
      throw new Error('Exhibit already reviewed');
    }
